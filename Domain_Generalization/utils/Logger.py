...
class Logger():
    ...
    def log(self, it, iters, losses, samples_right, total_samples):
        ...
        if it % self.update_f == 0:
            print("%d/%d of epoch %d/%d %s - acc %s [bs:%d]" % (it, iters, self.current_epoch, self.max_epochs, loss_string, acc_string, total_samples))
            # update tf log
            if self.tf_logger:
    ...
    def get_name_from_args(args):
        ...
        name = "eps%d_bs%d_lr%g_class%d_jigClass%d_jigWeight%g" % (args.epochs, args.batch_size, args.learning_rate, args.n_classes,30, 0.7)
